...
def clean_text(text: str) -> str:
    # Normalize smart quotes and special punctuation to ASCII equivalents
    replacements = {
        '\u2018': "'", '\u2019': "'",   # ' '  → '
        '\u201c': '"', '\u201d': '"',   # " "  → "
        '\u2013': '-', '\u2014': '-',   # – —  → -
...
SEED_URL = f"https://adaderana.lk/news.php?nid={start}"
session  = get_session_via_selenium(SEED_URL)

SLEEP_BETWEEN = 1      # seconds between requests (was 3)
REAUTH_EVERY  = 200    # re-solve Sucuri challenge every N articles

for i in range(start, end):
    current_url = f'https://adaderana.lk/news.php?nid={i}'
    ...
    headline_tag = page_soup.find("h1")
    ts_tag       = page_soup.find("p",   {"class": "news-datestamp"})
    content_tag  = page_soup.find("div", {"class": "news-content"})
    ...
    news = clean_text(content_tag.get_text().strip())
    ...
    if headline and news:
        ...
        out_dir     = os.path.join(".", "adaderana_english", date_folder)
