...
# CONFIG
# ======================
BASE_URL = "https://hirunews.lk/api/fetch_en_news.php"
BASE_DIR = "data"
SOURCE = "hirunews"
...
def scrape(category="General", max_pages=10000):
    ...
    for page in range(start_page, max_pages + 1):
        ...
        for item in data:
            try:
                art_id = int(item["eng_art_id"])
                date = item["eng_added_date"].split(" ")[0]

                article = {
                    "Source": SOURCE,
                    "Timestamp": item.get("eng_added_date", ""),
                    "Headline": item.get("eng_title", ""),
                    "News Content": clean_html(item.get("eng_story", "")),
                    "URL": "https://hirunews.lk/" + item.get("seourltitle", ""),
                    "Category": category,
                    "Parent URL": f"https://hirunews.lk/en/news_listing.php?category={category}",
                    "id": art_id
                }
...
if __name__ == "__main__":
    scrape(category="Business", max_pages=100000)